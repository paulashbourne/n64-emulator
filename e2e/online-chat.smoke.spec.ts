...
test('players can exchange chat messages inside an online session', async ({ page, browser }) => {
  ...
  const chatMessage = `hello-from-guest-${Date.now()}`;
  ...
  await expect(page.locator('.chat-list li strong').filter({ hasText: 'GuestTwo' })).toBeVisible({
    timeout: 15_000,
  });
