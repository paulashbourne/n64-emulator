...
test('host can kick a member and kicked user gets notified', async ({ page, browser }) => {
  ...
  await page
    ...
    .filter({ hasText: 'GuestTwo' })
    .getByRole('button', { name: 'Kick' })
    .click();

  await expect(page.getByText('Player 2: Open slot')).toBeVisible({ timeout: 15_000 });
  await expect(guestPage.getByText('You were removed by the host.')).toBeVisible({ timeout: 15_000 });
