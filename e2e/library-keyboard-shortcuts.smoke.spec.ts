import { expect, test } from '@playwright/test';

function buildValidRomBuffer(title: string, totalBytes: number): Buffer {
  const size = Math.max(0x80, totalBytes);
  const bytes = Buffer.alloc(size, 0);
  bytes[0] = 0x80;
  bytes[1] = 0x37;
  bytes[2] = 0x12;
  bytes[3] = 0x40;
  Buffer.from(title.padEnd(20, ' ').slice(0, 20), 'ascii').copy(bytes, 0x20);
  return bytes;
}

test('library keyboard shortcuts toggle view and favorites filter', async ({ page }) => {
  await page.goto('/');
  await page.setInputFiles('input[type="file"]', [
    {
      name: 'shortcut-alpha.z64',
      mimeType: 'application/octet-stream',
      buffer: buildValidRomBuffer('SHORTCUT ALPHA', 0x2200),
    },
    {
      name: 'shortcut-beta.z64',
      mimeType: 'application/octet-stream',
      buffer: buildValidRomBuffer('SHORTCUT BETA', 0x2400),
    },
  ]);

  await expect(page.locator('.rom-row')).toHaveCount(2, { timeout: 20_000 });
  await expect(page.locator('.rom-list')).not.toHaveClass(/rom-list-grid/);

  await page.keyboard.press('g');
  await expect(page.locator('.rom-list')).toHaveClass(/rom-list-grid/);
  await page.keyboard.press('g');
  await expect(page.locator('.rom-list')).not.toHaveClass(/rom-list-grid/);

  const alphaRow = page.locator('.rom-row').filter({ has: page.getByRole('heading', { name: 'SHORTCUT ALPHA' }) });
  await alphaRow.getByRole('button', { name: 'Favorite' }).click();
  await expect(alphaRow.getByRole('button', { name: 'Unfavorite' })).toBeVisible();

  await page.keyboard.press('f');
  await expect(page.locator('.rom-row')).toHaveCount(1);
  await expect(page.getByRole('heading', { name: 'SHORTCUT ALPHA' })).toBeVisible();

  await page.keyboard.press('f');
  await expect(page.locator('.rom-row')).toHaveCount(2);
});
