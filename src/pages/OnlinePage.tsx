...
export function OnlinePage() {
  ...
  useEffect(() => {
    if (!selectedRomId && roms.length > 0) {
      setSelectedRomId(roms[0].id);
    }
  }, [roms, selectedRomId]);

  const selectedRom = useMemo(
    () => roms.find((rom) => rom.id === selectedRomId),
    [roms, selectedRomId],
  );
      ...
      <section className="panel">
        ...
        <div className="online-form-grid">
          ...
          <label>
            ...
            >
              {roms.length === 0 ? <option value="">No ROM selected</option> : null}
              {roms.map((rom) => (
